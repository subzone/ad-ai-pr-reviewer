...
exports.selectSkillsForFile = selectSkillsForFile;
 ...
 */
function selectSkillsForFile(file, diff, requestedSkills, autoDetect = true) {
    const skills = [];
    const skillIds = new Set();
    // Add explicitly requested skills
    for (const skillId of requestedSkills) {
        const skill = exports.BUILTIN_SKILLS[skillId];
    ...
    if (autoDetect) {
        for (const skill of Object.values(exports.BUILTIN_SKILLS)) {
            if (skillIds.has(skill.id))
                continue; // Already added
            // Check file patterns
            if (skill.filePatterns) {
                const matches = skill.filePatterns.some(pattern => matchesPattern(file, pattern));
                if (matches) {
                    skills.push(skill);
                    skillIds.add(skill.id);
                    continue;
            ...
            if (skill.contentPatterns) {
                const matches = skill.contentPatterns.some(pattern => diff.toLowerCase().includes(pattern.toLowerCase()));
                if (matches) {
                    skills.push(skill);
                    skillIds.add(skill.id);
                }
...
function getSkillById(skillId) {
    return exports.BUILTIN_SKILLS[skillId];
}
/**
 * List all available skills
 */
function listAvailableSkills() {
    return Object.values(exports.BUILTIN_SKILLS);
}
/**
 * Get skill IDs from comma-separated string
 */
function parseSkillIds(skillsInput) {
    return skillsInput
        .split(',')
        .map(s => s.trim())
        .filter(s => s.length > 0);
 ...
 */
function buildSkillUserPrompt(skill, file, diff, options) {
    const lines = [];
    if (options.prTitle) {
 ...
 */
function validateSkillFindings(findings, diff, skill) {
    const accepted = [];
    const filtered = [];
 ...
 */
async function executeSkillsParallel(skills, file, diff, client, options) {
    console.log(`  Running ${skills.length} skill(s) for ${file}: ${skills.map(s => s.name).join(', ')}`);
    const results = await Promise.all(skills.map(skill => executeSkill(skill, file, diff, client, options)));
    // Log summary
