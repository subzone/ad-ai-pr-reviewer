...
exports.normalizeAzureEndpoint = normalizeAzureEndpoint;
        ...
        if (!lastHasText) {
            ...
            const toolContext = toolCalls.length > 0
                ? '\n\n## Additional Context (gathered via tools)\n\n' +
                    toolCalls.map(c => `**${c.tool}(${JSON.stringify(c.input)}):**\n${c.result}`).join('\n\n')
                : '';
            try {
                const finalParams = {
                    model,
                ...
                console.warn(`⚠️  Final summary request failed for ${file}: ` +
